...
const { findActiveBlock, formatTimeOf } = require("./tableBookingController");
 ...
 * the table. Mounted at /api/qr by routes/qrRoute.js, which owns the rate
 * limits; everything here is tenant-scoped from the token by
 * resolveTableScope, never from the URL or body.
 *
 * The public handlers take `:token`; the two staff handlers (generate,
 * dismiss) take a session.
 */
...
const scopedMenu = async (restaurantId, outletId) => {
  const docs = await Menu.find({
    ...
    isDeleted: false,
    // A table QR is served from the system snapshot, so it follows POS
    // visibility -- the PUBLISHED flags, applied inside projectMenus.
  });
...
const getTableByToken = async (req, res, next) => {
  try {
    ...
    const liveSession = await findActiveSessionByTable({ tableId: table._id, restaurantId });
...
const getSessionByToken = async (req, res, next) => {
  try {
    ...
    const activeSession = await findActiveSessionByTable({ tableId: table._id, restaurantId });
...
const addSessionItems = async (req, res, next) => {
  try {
    const { table, restaurantId, outletId } = req.scope;
    ...
    // (restaurant, table, requestId) index settles a race between two.
    const findByRequestId = () =>
      requestId ? Order.findOne({ restaurantId, table: table._id, requestId, isDeleted: { $ne: true } }) : null;

    let result;
    try {
      const placed = await findOrCreate({
        find: findByRequestId,
        create: async () => (await runWithSessionRetry(async (mongoSession) => {
    ...
    try {
      ...
      console.warn("[qr] socket emit failed:", socketErr.message);
    }
  ...
  try {
    ...
    if (gw.enabled && payable > 0) {
      try {
        ...
        console.warn("[qr] gateway order failed:", gwErr?.message || gwErr);
      }
    }
...
const paymentVerify = async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.warn("[qr] cashfree status check failed:", cfErr?.message || cfErr);
...
const callWaiter = async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.warn("[qr] waiter:called emit failed:", socketErr.message);
    }
...
const dismissWaiterCall = async (req, res, next) => {
  try {
    ...
    try {
      ...
      console.warn("[qr] waiter:cleared emit failed:", socketErr.message);
    }
