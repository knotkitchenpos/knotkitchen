 ...
 *
 * Regression test for a bug found during the code audit: the admin portal can
 * close a store temporarily, close it until a date, or delete it, but the POS
 * only ever rejected "suspended". Closed stores could therefore still be
 * logged into.
 */
...
test("an active store passes validation", async () => {
  ...
  const { res, err } = await call(userCtrl.checkStoreStatus, { storeId: "100001" });
  assert.ifError(err);
...
test("a pending store still passes validation (it has not been closed)", async () => {
  ...
  const { err } = await call(userCtrl.checkStoreStatus, { storeId: "100002" });
  assert.ifError(err);
});

test("a suspended store is rejected", async () => {
  makeStore("100003", { status: "suspended" });
  const { err } = await call(userCtrl.checkStoreStatus, { storeId: "100003" });
...
test("a temporarily closed store is rejected", async () => {
  ...
  const { err } = await call(userCtrl.checkStoreStatus, { storeId: "100004" });
  assert.equal(err.status, 400);
...
test("a store closed until a future date is rejected", async () => {
  ...
  const { err } = await call(userCtrl.checkStoreStatus, { storeId: "100005" });
  assert.equal(err.status, 400);
...
test("a store whose closure date has passed is allowed back in", async () => {
  ...
  const { err } = await call(userCtrl.checkStoreStatus, { storeId: "100006" });
...
test("a deleted store is rejected", async () => {
  ...
  const { err } = await call(userCtrl.checkStoreStatus, { storeId: "100007" });
  assert.equal(err.status, 400);
  assert.match(err.message, /no longer available/i);
});

test("closure is enforced on every store entry point, not just the status lookup", async () => {
