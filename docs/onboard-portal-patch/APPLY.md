# Onboarding portal → CSD: the integration patch

Everything `knotkitchenpos/onboard` needs so CSD can turn a signed agreement
into a store.

**Status: written against the real repo, applied to a clone, and verified
running.** Base commit `a719b58` ("fix: close authN/authZ, path-traversal, and
XSS gaps in onboarding portal"), the current `main`.

## Apply it

```bash
git apply onboard-portal.patch
```

`onboard-portal.patch` adds `server/middleware/serviceAuth.js` and edits
`server/routes/api.js` and `server/server.js` — 59 inserted lines, 10 removed.
