...
export const printReceipt = ({ cartData, customerData, total, tax, totalPriceWithTax }) => {
  ...
  const itemsHTML = cartData
    .map(
      (item) =>
        `<tr><td style="padding:6px 0;font-size:12px;">${item.name}</td><td style="padding:6px 0;font-size:12px;text-align:center;">x${item.quantity}</td><td style="padding:6px 0;font-size:12px;text-align:right;">Rs.${item.price.toFixed(2)}</td></tr>`
    )
    .join("");
