...
export const printReceipt = ({
  ...
  const itemsHTML = safeCartData
    .map(
      (item) =>
        `<tr><td style="padding:6px 0;font-size:12px;">${esc(item.name)}</td><td style="padding:6px 0;font-size:12px;text-align:center;">x${Number(item.quantity) || 0}</td><td style="padding:6px 0;font-size:12px;text-align:right;">Rs.${Number(item.price || 0).toFixed(2)}</td></tr>`
    )
    .join("");
